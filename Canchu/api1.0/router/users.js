...
router.get('/search', verifyAccesstoken, async(req, res) => {
    ...
    console.log(keyword);
    let mysQuery = 'SELECT users.id AS user_id, name, picture, friendship.id AS friendship_id, is_friend, sender_id, receiver_id FROM users LEFT JOIN friendship ON users.id = friendship.sender_id OR users.id = friendship.receiver_id WHERE name LIKE "%user%"';
